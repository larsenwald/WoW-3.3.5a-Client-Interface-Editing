...
function AccountLogin_OnLoad(self)
	...
	PasswordEditBox:SetBackdropBorderColor(backdropColor[1], backdropColor[2], backdropColor[3]);
	PasswordEditBox:SetBackdropColor(backdropColor[4], backdropColor[5], backdropColor[6]);
...
function AccountLogin_OnShow(self)
	...
	UsernameEditBox:SetText(accountName);
...
function AccountLogin_OnHide(self)
	...
	StopAllSFX( 1.0 );
	if ( not RememberMeCheckBox:GetChecked() ) then
		SetSavedAccountList("");
	end
end
--]]
...
function AccountLogin_Login()
	PlaySound("gsLogin");
	DefaultServerLogin(UsernameEditBox:GetText(), PasswordEditBox:GetText());
	PasswordEditBox:SetText("");
	
	if ( RememberMeCheckButton:GetChecked() ) then
		SetSavedAccountName(UsernameEditBox:GetText());
	else
		SetSavedAccountName("");
		SetUsesToken(false);
	end
...
function TokenEntryOkayButton_OnEvent(self, event)
	if (event == "PLAYER_ENTER_TOKEN") then
		if ( RememberMeCheckBox:GetChecked() ) then
...
function AccountLogin_SetupAccountListDDL()
	if ( GetSavedAccountName() ~= "" and GetSavedAccountList() ~= "" ) then
		--PasswordEditBox:SetPoint("BOTTOM", 0, 255);
		--AccountLoginLoginButton:SetPoint("BOTTOM", 0, 150);
		--AccountLoginDropDown:Show();
	else
		--PasswordEditBox:SetPoint("BOTTOM", 0, 275);
